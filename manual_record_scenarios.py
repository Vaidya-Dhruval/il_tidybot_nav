...
def load_scenarios(path: str):
    ...
    if "scenarios" not in data or not isinstance(data["scenarios"], list):
        raise ValueError(f"Scenario file must contain a top-level 'scenarios' list: {path}")

    scenarios = []
    for i, sc in enumerate(data["scenarios"]):
        for key in ["name", "x", "y", "yaw"]:
            if key not in sc:
                raise ValueError(f"Scenario index {i} missing required key '{key}'")
        scenarios.append(
            {
                "name": str(sc["name"]),
                "x": float(sc["x"]),
                "y": float(sc["y"]),
                "yaw": float(sc["yaw"]),
                "max_steps": int(sc.get("max_steps", ENV.max_steps)),
            }
        )
    if not scenarios:
...
class EpisodeBuffer:
    ...
    def summary(self):
        ...
        return {
            ...
            "final_hold_counter": int(self.hold_counter[-1]),
...
class ShardWriter:
    def __init__(self, out_dir: Path, shard_size: int, prefix: str = "manual_stage0_multi", save_images: bool = False):
...
def print_controls():
    print("")
    print("Manual scenario-record controls")
    print("-------------------------------")
    ...
    print("r : reset same scenario and discard current episode")
...
def episode_end_prompt(summary):
    ...
    print(
        ...
        f"hold={summary['final_hold_counter']}"
    )
    print("Press 'k' keep, 'x' discard, 'r' retry same, 'n' next scenario, 'b' prev scenario, or 'esc' quit.")


def build_env_for_scenario(base_params, scenario, render_mode="human"):
    p = replace(base_params, max_steps=int(scenario.get("max_steps", base_params.max_steps)))
    return TidybotNavEnvV12Stage0(p, render_mode=render_mode)


def reset_scenario(env, scenario, seed=None):
    return env.reset(
        seed=seed,
        options={
            "manual_spawn": {
...
def main():
    ...
    ap.add_argument("--scenario_file", type=str, required=True)
    ap.add_argument("--out_dir", type=str, required=True)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--default_max_steps", type=int, default=1500)
    ap.add_argument("--pulse", type=float, default=0.60)
    ...
    ap.add_argument("--yaw_tol", type=float, default=0.25)
    args = ap.parse_args()
    ...
    base_params = replace(
        ENV,
        max_steps=int(args.default_max_steps),
        prepose_yaw_tolerance=float(args.yaw_tol),
    )

    writer = ShardWriter(
        out_dir=Path(args.out_dir),
        shard_size=int(args.shard_size),
        prefix="manual_stage0_multi",
        save_images=bool(args.save_images),
    )

    scenario_idx = 0
    scenario = scenarios[scenario_idx]
    env = build_env_for_scenario(base_params, scenario, render_mode="human")

    print("[manual_record_scenarios] starting")
    print(f"[manual_record_scenarios] MUJOCO_GL={os.environ.get('MUJOCO_GL')}")
    print(f"[manual_record_scenarios] scenario_file={args.scenario_file}")
    print(f"[manual_record_scenarios] scenarios={len(scenarios)}")
    print(f"[manual_record_scenarios] default_max_steps={args.default_max_steps} yaw_tol={args.yaw_tol}")
    print(f"[manual_record_scenarios] current scenario={scenario['name']}")

    obs, info = reset_scenario(env, scenario, seed=args.seed)
    print_controls()
    ...
    def switch_to_current_scenario(force_rebuild=False):
        nonlocal env, obs, info, action, t_ep, prev_in_goal, scenario

        desired_max_steps = int(scenario.get("max_steps", base_params.max_steps))
        current_max_steps = int(env.p.max_steps)

        need_rebuild = bool(force_rebuild) or (current_max_steps != desired_max_steps)

        if need_rebuild:
            print(f"[env] rebuilding for scenario={scenario['name']} max_steps={desired_max_steps}")
            try:
                env.close()
            except Exception:
                pass
            ...
            print(f"[env] reusing existing viewer for scenario={scenario['name']}")
        ...
        while True:
            ...
            if key is not None:
                if ord(key) == 27:
                    ...
                    print(f"\n[reset] retry scenario={scenario['name']}")
                    epbuf.clear()
                    total_discarded += 1
                    switch_to_current_scenario(force_rebuild=False)
                    continue
                elif key == "n":
            ...
            base_th = float(info.get("base_th", np.nan))
            target_yaw = float(info.get("prepose_yaw", env.p.prepose_yaw))
            base_th_deg = np.degrees(base_th)
            target_yaw_deg = np.degrees(target_yaw)
            yaw_err_deg = np.degrees(yaw_err)
            yaw_tol_deg = np.degrees(env.p.prepose_yaw_tolerance)

            if abs(yaw_err_deg) <= yaw_tol_deg:
                yaw_hint = "YAW_OK"
            elif yaw_err_deg > 0.0:
                yaw_hint = f"TURN_LEFT {abs(yaw_err_deg):.1f}deg"
            else:
                yaw_hint = f"TURN_RIGHT {abs(yaw_err_deg):.1f}deg"

            if bool(in_goal) and not prev_in_goal:
                print(f"\n[enter-goal] sc={scenario['name']} d={d_anchor:.3f} yaw={yaw_err_deg:+.1f}deg HOLD={hold_counter}/{env.p.hold_steps}")
            elif (not bool(in_goal)) and prev_in_goal:
                print(f"\n[exit-goal]  sc={scenario['name']} d={d_anchor:.3f} yaw={yaw_err_deg:+.1f}deg HOLD={hold_counter}/{env.p.hold_steps}")
            ...
            print_now = (t_ep % 10 == 0) or (hold_counter > 0)
            if print_now:
                print(
                    f"\r[sc={scenario['name']} ep={episode_id:04d} step={t_ep:04d}] "
                    f"a=({action[0]:+.2f},{action[1]:+.2f},{action[2]:+.2f}) "
                    f"d={d_anchor:.3f} POS_OK={pos_ok} | "
                    f"yaw={base_th_deg:+6.1f}deg target={target_yaw_deg:+6.1f}deg "
                    f"err={yaw_err_deg:+6.1f}deg tol=±{yaw_tol_deg:.1f}deg "
                    f"YAW_OK={yaw_ok} {yaw_hint} | "
                    f"IN_GOAL={in_goal} HOLD={hold_counter:02d}/{env.p.hold_steps:02d} "
            ...
            if done:
                ...
                if decision == "keep":
                    ...
                    total_kept += 1
                    print(f"[keep] scenario={scenario['name']} episode={episode_id} kept={total_kept} discarded={total_discarded}")
                    episode_id += 1
                elif decision == "discard":
                    total_discarded += 1
                    print(f"[discard] scenario={scenario['name']} kept={total_kept} discarded={total_discarded}")
                    episode_id += 1
                elif decision == "retry":
                    total_discarded += 1
                    print(f"[retry] scenario={scenario['name']}")
                elif decision == "next":
                    total_discarded += 1
                    scenario_idx = (scenario_idx + 1) % len(scenarios)
                    scenario = scenarios[scenario_idx]
                    print(f"[next] scenario={scenario['name']}")
                elif decision == "prev":
                    total_discarded += 1
                    scenario_idx = (scenario_idx - 1) % len(scenarios)
                    scenario = scenarios[scenario_idx]
                    print(f"[prev] scenario={scenario['name']}")
                elif decision == "quit":
                    print("[quit] stopping")
                    break
